...
/* We want exact 32bits; uint_fast32_t sometimes assign a type larger than
 * 32bits */
#define SHA1_WORD uint32_t

SHA1_WORD
sha1_f(uint8_t t, SHA1_WORD B, SHA1_WORD C, SHA1_WORD D)
{
	if(t <= 19) {
		return (B & C) | ((~B) & D);
	} else if(t <= 39) {
...
uint8_t *
sha1_pad(const void *msg, size_t size, size_t *newSize)
{
	if(!msg) {
		return 0;
	}
	...
	/*
   * This code relies too much on the endianess of the system, so we won't be
   * using it uint64_t* ref = (uint64_t*) (newArr + size + toPad - 8); ref =
   * size * 8;
   */
...
uint8_t *
method_one(const void *msg, size_t size)
{
...
uint8_t *
method_two(const void *msg, size_t size)
{
	...
	for(int i = 0; i < messageSize; i += 64) {
		...
		for(t = 0; t < 80; t++) {
			int s = t & MASK;
			if(t >= 16) {
				W[s] = sha1_Sn(W[(s + 13) & MASK] ^ W[(s + 8) & MASK] ^
						W[(s + 2) & MASK] ^ W[s],
					1);
			}
